...
class QuantLinearAdapter(nn.Module):
    ...
    def __init__(self, bits, groupsize, infeatures, outfeatures, bias, bottleneck_size=256, non_linearity='tanh', lora_dropout=0.0, scaling=1.0, init_weights=None, adapter_type='output_adapter'):
        super().__init__()
        ...
        if bias:
            ...
            self.bias = None
        self.equip_with_adapter(bottleneck_size, non_linearity, infeatures, outfeatures, lora_dropout, scaling, adapter_type) # this can be excute out of the class if necessary
        
    def equip_with_adapter(self, bottleneck_size, non_linearity, infeatures, outfeatures, lora_dropout, scaling, adapter_type):
        ####### adapter here
        if bottleneck_size > 0:
            self.bottleneck1 = nn.Linear(infeatures, bottleneck_size, bias=False)
            self.bottleneck2 = nn.Linear(bottleneck_size, outfeatures, bias=False)
            self.scaling = scaling
        self.adapter_type = adapter_type
        self.act_fn = ACT2FN[non_linearity]
            ...
            if self.adapter_type == 'mh_adapter':
                ...
                if not torch.is_autocast_enabled():
                    ...
                    if x.dtype != torch.float32:
                        x = x.float()
                    output = self.bottleneck2(self.act_fn(self.bottleneck1(self.lora_dropout(x)))).to(expected_dtype) * self.scaling
                    output = output + residual
                else:
                    output = self.bottleneck2(self.act_fn(self.bottleneck1(self.lora_dropout(x)))) * self.scaling
                l = nn.Linear(self.outfeatures, self.outfeatures)
                output = l(output)
            elif self.adapter_type == 'output_adapter':
                # for output_adapter, x will pass the linear layer first and then the adapter
                if not torch.is_autocast_enabled():
                    l = nn.Linear(self.infeatures, self.infeatures)
                    x = l(x)
                ...
                if not torch.is_autocast_enabled():
                    ...
                    output = self.bottleneck2(self.act_fn(self.bottleneck1(self.lora_dropout(x)))) * self.scaling
        #### add adapter results
        return out.reshape(out_shape) + output


def make_quant_linear(module, names, bits, groupsize, name=''):
    if isinstance(module, QuantLinearAdapter):
        return
    for attr in dir(module):
        ...
        if name1 in names:
            delattr(module, attr)
            setattr(module, attr, QuantLinearAdapter(bits, groupsize, tmp.in_features, tmp.out_features, tmp.bias is not None))
...
def autotune_warmup_linear(model, transpose=False):
    ...
    for _, m in model.named_modules():
        if not isinstance(m, QuantLinearAdapter):
            continue
